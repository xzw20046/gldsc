...
plot.bias.1.1<-ggplot(data=plot.data,aes(x=true_bias,y=bias, fill=Methods))+
  ...
  stat_summary(fun.y = mean, color = "darkred", position = position_dodge(0.75),
               geom = "point", shape = c(18), size = 3,
               show.legend = FALSE)+geom_abline(slope=0, intercept=c(1.1),linetype = "dashed")+
  xlab('Causal rate')+ylab('Estimated Inflation')+
  theme_light()+
  scale_fill_manual(values = c('#F8766D','#619CFF'))+
  theme(axis.title.x=element_blank(),
...
plot.bias.1.05<-ggplot(data=plot.data,aes(x=true_bias,y=bias, fill=Methods))+
  ...
  stat_summary(fun.y = mean, color = "darkred", position = position_dodge(0.75),
               geom = "point", shape = c(18), size = 3,
               show.legend = FALSE)+geom_abline(slope=0, intercept=c(1.05),linetype = "dashed")+
  xlab('Causal rate')+ylab('Estimated Inflation')+
  theme_light()+
  scale_fill_manual(values = c('#F8766D','#619CFF'))+
  theme(axis.title.x=element_blank(),
...
plot.confound.data[,'N']<-rep(c('n = 20,000','n = 10,000',
...
plot.data.confounding<-plot.confound.data[which(plot.confound.data$h2=='0.4'&
                                plot.confound.data$causal=='0.05'),]
...
#plto of e
mean.data<-aggregate(plot.confound.data[,c(1)],list(plot.confound.data$group.box,plot.confound.data$Inflation,
                                            plot.confound.data$Enrichment),mean)
sd.data<-aggregate(plot.confound.data[,c(1)],list(plot.confound.data$group.box,plot.confound.data$Inflation,
                                          plot.confound.data$Enrichment),sd)
    ...
    for (heritability in c(0.4)) {
      for (DHSenrichment in c(1,3)) {
        for (confound in c(1,1.05,1.1,1.2)) {
          ...
          plot.of.lambda.temp[,'confound']=confound
...
names(plot.of.lambdaE)=c('Org','Iter','N','h2','poly','True_E','Confound')
plot(plot.of.lambdaE$Org~plot.of.lambdaE$Iter)

plot.of.lambdaE[,'Class']=paste0(plot.of.lambdaE$N,'_',plot.of.lambdaE$Confound)

ggplot(plot.of.lambdaE, aes(x=Org, y=Iter)) +
  geom_point(size=1.5)+ geom_abline(slope=1, intercept=c(0),linetype = "dashed")+facet_wrap(. ~ `Class`,nrow = 4)+
  geom_smooth(method = "lm", fill = NA)+coord_cartesian(ylim=c(-2, 10),xlim = c(-2,10))
...
View(cbind(aggregate(plot.of.lambdaE$Org,list(plot.of.lambdaE$Confound,plot.of.lambdaE$True_E,plot.of.lambdaE$N),mean),
           aggregate(plot.of.lambdaE$Iter,list(plot.of.lambdaE$Confound,plot.of.lambdaE$True_E,plot.of.lambdaE$N),mean)[,4]))
