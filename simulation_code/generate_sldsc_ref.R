...
#output
write.table(anno2,paste0(outpath,'/baseline.10.annot'),quote = F,row.names = F,sep = '\t')
write.table(ldsc.file2,paste0(outpath,'/baseline.10.l2.ldscore'),quote = F,row.names = F,sep = '\t')
write.table(t(w.file),paste0(outpath,'/baseline.10.l2.M_5_50'),quote = F,row.names = F,col.names = F,sep = '\t')
